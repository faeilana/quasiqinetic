...
var GLctx;
// Lazy GL context init: CPU delegate skips Browser.createContext during setup,
// but the inference pipeline still calls glActiveTexture for frame preprocessing.
// If GLctx is unset when the first GL call arrives, create the context now.
function _ensureGLctx() {
  if (GLctx) return;
  var c = Module.canvas;
  console.log('[mpatch] _ensureGLctx: Module.canvas=', !!c);
  if (!c) return;
  // Pre-set getContextSafariWebGL2Fixed so GL.createContext skips its
  // broken instanceof-based Safari patch (which returns null for webgl2
  // on Safari where WebGL2RenderingContext extends WebGLRenderingContext).
  if (!c.getContextSafariWebGL2Fixed) {
    c.getContextSafariWebGL2Fixed = c.getContext.bind(c);
  }
  var h = GL.createContext(c, { majorVersion: 2, antialias: false, alpha: false, enableExtensionsByDefault: true });
  if (!h) h = GL.createContext(c, { majorVersion: 1, antialias: false, alpha: false, enableExtensionsByDefault: true });
  console.log('[mpatch] GL.createContext handle=', h);
  if (h) {
    GL.makeContextCurrent(h);
    console.log('[mpatch] GLctx set:', !!GLctx);
  } else {
    // WebGL unavailable — use a no-op Proxy so GL calls don't crash the WASM.
    // CPU-delegate inference still runs; only GPU profiling calls are silenced.
    console.warn('[mpatch] WebGL context creation failed, installing no-op Proxy for GLctx');
    GLctx = new Proxy({}, {
      get: function(_, p) {
        if (p === 'canvas') return c;
        if (p === 'getParameter') return function() { return 0; };
        if (p === 'getSupportedExtensions') return function() { return []; };
        if (p === 'getExtension') return function() { return null; };
        if (p === 'checkFramebufferStatus') return function() { return 36053; };
...
var _emscripten_glActiveTexture = x0 => { _ensureGLctx(); GLctx?.activeTexture(x0); };
